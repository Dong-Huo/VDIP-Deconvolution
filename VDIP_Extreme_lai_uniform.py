...
def get_negative_kl(out_x, out_k):
    ...
    # dark channel prior
    image_E = image_E.unsqueeze(1)
    image_STD = image_STD.unsqueeze(1)

    E_square = torch.square(image_E) + torch.square(image_STD)
    S_square = torch.sqrt(
        4 * torch.square(image_E) * torch.square(image_STD) + 2 * torch.square(image_STD) * torch.square(
            image_STD) + 1e-32)

    eps_dcp = torch.randn_like(image_E)

    dcp = -F.max_pool3d(-(E_square + eps_dcp * S_square), (3, 35, 35), stride=1)
    epsilon = dcp.detach() + bias

    dcp = dcp / epsilon

    # bright channel prior
    E_square = torch.square(torch.ones_like(image_E) - image_E) + torch.square(image_STD)
    S_square = torch.sqrt(
        4 * torch.square(image_STD) + 4 * torch.square(image_E) * torch.square(image_STD) + 2 * torch.square(
            image_STD) * torch.square(image_STD) + 1e-32)

    eps_ecp = torch.randn_like(image_E)

    bcp = -F.max_pool3d(-(E_square + eps_ecp * S_square), (3, 35, 35), stride=1)
    epsilon = bcp.detach() + bias

    bcp = bcp / epsilon

    # extreme channel prior
    term_2 = torch.sum(dcp) / 4 + torch.sum(bcp) / 4

    # kernel normalization
    term_3 = torch.sum(torch.square(out_k)) / 2

    return term_1 / (out_x.shape[2] * out_x.shape[3] * 3) - term_2 / (
            (out_x.shape[2] - 34) * (out_x.shape[3] - 34)) - term_3 * 10


def get_prior(out_x, out_k):
    image_E = out_x[:, 0:3, :, :]
    ...
    dcp = -F.max_pool3d(-torch.square(image_E), (3, 35, 35), stride=1)
    epsilon = dcp.detach() + bias

    dcp = dcp / epsilon

    # bright channel prior
    image_E = torch.ones_like(image_E) - image_E
    bcp = -F.max_pool3d(-torch.square(image_E), (3, 35, 35), stride=1)
    epsilon = bcp.detach() + bias
    ...
    return - term_2 / ((out_x.shape[2] - 34) * (out_x.shape[3] - 34)) - term_3 * 10
...
def checkNaN(optimizer):
    for group in optimizer.param_groups:
        for param in group["params"]:
            if param.grad is not None:
                if True in torch.isnan(param.grad.data):
                    return True
    return False
...
for f in files_source:
    ...
    for step in tqdm(range(opt.num_iter)):
        ...
        if step < 2000:
            ...
            # prior = get_prior(out_x, out_k)
            sampling_expectation = get_sampling_expectation(out_x, out_k, initial_img_target, opt.num_sampling, step,
                                                            loss_function="ssim")

            total_loss = -sampling_expectation

            # total_loss = -sampling_expectation - prior * current_noise_sigma
            # print("prior: " + str(prior.item()))
            print("sampling_expectation: " + str(sampling_expectation.item()))
        ...
        # if current_noise_sigma <= 0.001:
        #     current_noise_sigma = 0.001

        # current_noise_sigma = opt.noise_sigma - opt.noise_sigma / 1000 * (step + 1)

        total_loss.backward()
        if checkNaN(optimizer):
            continue
        optimizer.step()
